...
mymainmenu = {
   {"Notes", "/opt/ibm/lotus/notes/notes","/opt/ibm/lotus/notes/notes.png"},
   {"Firefox", "/usr/bin/firefox","/usr/share/pixmaps/firefox-3.0.png"},
   {"Terminal", terminal, "/usr/share/pixmaps/gnome-term.png" },
...
for s = 1, screen.count() do
    ...
    mypromptbox[s] = widget({ type = "textbox", fg="#ffaaaa", name = "mypromptbox" .. s, align = "left" })
...
--keybinding({ modkey }, "p", function () {awful.spawn, "Run: "} end):add()
...
keybinding({ modkey }, "p", function ()
                                 awful.prompt.run({ prompt = "<span color='#ffaaaa'>Run: </span>" }, mypromptbox[mouse.screen], awful.util.spawn, awful.completion.bash,
